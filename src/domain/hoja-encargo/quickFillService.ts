 ...
 */
export async function saveQuickFillClient(dto: Omit<QuickFillClientDTO, 'lastUsedAt' | 'id'>): Promise<void> {
    await db.transaction('rw', db.quickFillClients, async () => {
        ...
        if (dto.nif && dto.nif.trim() !== '') {
            ...
            const match = await db.quickFillClients.filter(c => c.nombre.toUpperCase() === dto.nombre.toUpperCase()).first();
            if (match) existingId = match.id;
        }
