...
import { countOfflineExpedienteWrites, upsertOfflineExpedienteWrite } from "./lib/offlineQueue";
...
function parseCE3XXml(xmlText: string): ParsedCE3X {
  ...
  const docInmueble =
    doc.querySelector("IdentificacionEdificio") ||
    doc.querySelector("DatosDelInmueble") ||
    doc.querySelector("Entrada") ||
    doc;
  ...
  const elementosHuecos = [
    ...doc.querySelectorAll("HuecosYLucernarios Elemento, HuecosyLucernarios Elemento"),
  ];
...
const ZONAS_CLIMATICAS = Object.entries(VALORES_G).map(([zona, g]) => {
  const NOMBRES: Record<string, string> = {
    ...
    A2: "A2",
    A3: "A3 — Málaga, Almería",
    A4: "A4 — Cádiz",
    B2: "B2",
    B3: "B3 — Valencia, Alicante",
    B4: "B4 — Sevilla, Córdoba",
    C1: "C1 — Santander, Bilbao",
    C2: "C2 — Barcelona",
    C3: "C3 — Granada",
    C4: "C4 — Cáceres",
    D1: "D1 — Vitoria, Pamplona",
    D2: "D2 — Valladolid, Zamora",
    D3: "D3 — Madrid, Toledo",
    E1: "E1 — Burgos, León, Soria",
    E2: "E2",
    E3: "E3",
  };
...
const ENABLE_EXPEDIENTE_MVP_RPC =
  String(import.meta.env.VITE_EXPEDIENTES_RPC_ENABLED ?? "false").toLowerCase() === "true";
...
const QUICK_LAYER_PRESETS: Record<
  QuickLayerPresetId,
  { nombre: string; r: number; espesor: number; lambda: number }
> = {
  ...
  yeso_023: {
    nombre: "Yeso, de alta dureza 900 < d < 1200",
    r: 0.023,
    espesor: 0.01,
    lambda: 0.43,
  },
  yeso_018: {
    nombre: "Yeso, de alta dureza 1200 < d < 1500",
    r: 0.018,
    espesor: 0.01,
    lambda: 0.56,
  },
...
const COMMON_LAYER_SETS: Record<
  CommonLayerSetId,
  Array<{ preset: QuickLayerPresetId; esNueva: boolean }>
> = {
...
function mergeDraftPayload(
  base: CertificateDraftPayload,
  incoming: CertificateDraftPayload
...
function pickMostRecentDraft(
  a: CertificateDraftPayload,
  b: CertificateDraftPayload
): CertificateDraftPayload {
...
function pickMostRecentIndexItem(
  a: CertificateDraftIndexItem,
  b: CertificateDraftIndexItem
): CertificateDraftIndexItem {
  const aTime = Date.parse(a.updatedAt || "");
  const bTime = Date.parse(b.updatedAt || "");
  if (Number.isFinite(aTime) && Number.isFinite(bTime)) {
    return bTime >= aTime ? b : a;
...
export function CalculadoraTermica() {
  ...
  const [catastroVerificationBanner, setCatastroVerificationBanner] =
    useState<CatastroVerificationBanner | null>(null);
  ...
  const [pendingConflictResolution, setPendingConflictResolution] =
    useState<ExpedienteNeedsResolutionDetail | null>(null);
  ...
  const currentCalcFingerprint = useMemo(
    () =>
  ...
  const resultadoDesactualizado =
    Boolean(resultado) && currentCalcFingerprint !== lastCalculatedFingerprint;
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      EXPEDIENTE_NEEDS_RESOLUTION_EVENT,
      handleNeedsResolution as EventListener
    );
    return () => {
  ...
  useEffect(() => {
    ...
    const stop = startExpedienteMvpSyncLoop((report, reason: SyncReason) => {
      ...
      if (report.synced > 0) {
        setDraftMsg(
          `Sincronización SQL MVP: ${report.synced} pendiente(s) enviados. Pendientes: ${report.pending}.`
        );
      }
  ...
  useEffect(() => {
    ...
    setCapas((prevCapas) =>
      prevCapas.map((capa, idx) => {
        ...
        if (capa.lambda_val === defaultMat.lambda_w_mk && capa.nombre.includes(defaultMat.marca))
          return capa;
  ...
  const reindexRecordAfterRemove = <T,>(
  ...
  const addCapa = (esNueva: boolean) => {
    setCapas((prev) => [
      ...prev,
      { nombre: "", espesor: 0, lambda_val: 0, r_valor: 0, es_nueva: esNueva },
    ]);
  };
  ...
  const loadSupafilFichaTecnica = async () => {
    ...
    try {
      ...
      setDraftError(
        "No se pudo cargar la ficha técnica SUPAFIL automática. Puedes subirla manualmente en Capturas."
      );
    }
  };
  ...
  const updateCapa = <K extends keyof CapaMaterial>(
    idx: number,
    field: K,
    value: CapaMaterial[K]
  ) => {
    setCapas((prev) => {
      const updated = [...prev];
      updated[idx] = {
  ...
  const guardarCliente = async () => {
    ...
    if (!dni) {
      setDniLookupMsg("Introduce un DNI para guardar el cliente.");
      return;
    }
    if (!clienteFirstName.trim() || !clienteLastName1.trim()) {
      setDniLookupMsg("Introduce al menos primer nombre y primer apellido.");
      return;
    }
    ...
    try {
      ...
      setDniLookupMsg(
        `✅ Cliente ${first_name} ${last_name_1} ${verb} en local y encolado para sync.`
      );
    } catch (err: any) {
  ...
  const verificarCatastroDesdeDatos = async ({
    ...
    if (climateCheck.zone !== null) {
      ...
      if (xmlZona && climateCheck.zone !== xmlZona && zoneKnown) {
        climateParts.push(
          `⚠️ Zona: CE3X ${xmlZona} vs Catastro ${climateCheck.zone} (actualizada).`
        );
      } else if (xmlZona) {
        climateParts.push(`Zona: ${climateCheck.zone} (coincide con CE3X).`);
      } else {
        climateParts.push(
          `Zona automática: ${climateCheck.zone}${climateCheck.zoneProvince ? ` (${climateCheck.zoneProvince})` : ""}.`
        );
      }
    ...
    if (catastroCheck.error) {
      ...
      if (multiple) {
        ...
        if (rcNormalized.length === 20) {
          try {
            ...
            if (loints.length > 0) {
              const l = loints[0];
              const parts = [];
              if (l.bloque && l.bloque !== "N/D" && l.bloque !== "-" && l.bloque !== "—")
                parts.push(`Blq. ${l.bloque}`);
              if (l.escalera && l.escalera !== "N/D" && l.escalera !== "-" && l.escalera !== "—")
                parts.push(`Esc. ${l.escalera}`);
              if (l.planta && l.planta !== "N/D" && l.planta !== "-" && l.planta !== "—")
                parts.push(`Pl. ${l.planta}`);
              if (l.puerta && l.puerta !== "N/D" && l.puerta !== "-" && l.puerta !== "—")
                parts.push(`Pt. ${l.puerta}`);
        ...
        if (mismatches.length > 0) {
          ...
          locationTone = "ok";
          locationMsg =
            "✅ Ubicación verificada: dirección, municipio y provincia coinciden con Catastro.";
        } else {
          locationTone = "info";
    ...
    try {
      ...
      if (parsed.rc) {
        ...
        setCatastroVerificationBanner({
          tone: "warning",
          message:
            "⚠️ XML sin referencia catastral: no se pudo verificar Catastro automáticamente.",
        });
      }
    } catch {
      setClienteFirstName("");
  ...
  const calcular = async () => {
    ...
    try {
      ...
      const applyOverrides = (base: ResultadoTermico): ResultadoTermico => {
        ...
        const ahorroFinal =
          uiFinal > ufFinal ? Math.round((uiFinal - ufFinal) * supActuacion * gValue) : 0;
      ...
      if (isCloudCalculation) {
        try {
          ...
          const normalizedInforme =
            typeof informe === "string" && informe.trim() ? informe.trim() : null;
          ...
          const message = normalizedMessage.includes("401")
  ...
  const getResultadoActualizado = (actionLabel: string): ResultadoTermico | null => {
    ...
    if (resultadoDesactualizado) {
      setDraftError(
        `Has modificado datos tras el último cálculo. Pulsa "Calcular Ahorro Energético" antes de ${actionLabel}.`
      );
      return null;
    }
  ...
  const copiarInformeCloud = async () => {
    ...
    if (!cloudReportText) {
      setDraftError(
        "No hay informe cloud disponible todavía. Ejecuta un cálculo en modo On-Cloud."
      );
      return;
    }
...
*/
  ...
  const registrarCertificadoEmitido = async (
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from("work_photos")
        .upload(storagePath, generatedPdf.blob, {
          upsert: true,
      ...
      setDraftError(
        `PDF generado, pero no se pudo guardar en historial cloud: ${error?.message ?? "error desconocido"}`
      );
      return false;
    }
  };
  ...
  const generarCertificadoIntelliaPDF = async () => {
    ...
    try {
      ...
      setDraftError(null);
      const registered = await registrarCertificadoEmitido(
        "intellia_pdf",
        generatedPdf,
        resultadoActual
      );
  ...
  const generarDocumentoWord = async (fechaExport?: string) => {
    ...
    try {
      setDraftMsg(
        "Generando documento Word... (esto puede tardar unos segundos si hay imágenes grandes)"
      );
      ...
      await generarCertificadoE1_3_5_DOCX({
        ...
        clienteNombre: [clienteFirstName, clienteMiddleName, clienteLastName1, clienteLastName2]
          .filter(Boolean)
          .join(" "),
        direccionInmueble,
        municipioInmueble,
...
*/
  ...
  const clearLocalCalcMemory = () => {
    ...
    setXmlImportMsg(
      "Memoria local limpiada. Mantienes la sesión online, pero este formulario vuelve a estado manual."
    );
  };
  ...
  const normalizeDraftIndexItems = (
    items: CertificateDraftIndexItem[]
  ): CertificateDraftIndexItem[] => {
    return items
      ...
      .map((it) => {
        ...
        return {
          ...
          updatedAt:
            typeof it.updatedAt === "string" && it.updatedAt.trim()
              ? it.updatedAt
              : new Date().toISOString(),
  ...
  const normalizeIssuedCertificateRecords = (
    items: IssuedCertificateRecord[]
  ): IssuedCertificateRecord[] => {
    return items
      .filter((it) => typeof it?.rc === "string")
      .map((it) => {
        const normalizedRc = normalizeRc(String(it.rc ?? ""));
        const type: IssuedCertificateType =
          it.type === "anexo_e1_pdf" ? "anexo_e1_pdf" : "intellia_pdf";
        const fileName = sanitizeSegmentForPath(String(it.fileName ?? ""), `${type}.pdf`);
        const storagePath =
          typeof it.storagePath === "string" && it.storagePath.trim() ? it.storagePath : "";
        return {
          id:
          ...
          storagePath,
          issuedAt:
            typeof it.issuedAt === "string" && it.issuedAt.trim()
              ? it.issuedAt
              : new Date().toISOString(),
          clienteNombre: typeof it.clienteNombre === "string" ? it.clienteNombre : "",
          clienteDni: typeof it.clienteDni === "string" ? it.clienteDni : "",
  ...
  const mergeDraftIndexes = (
    ...groups: CertificateDraftIndexItem[][]
  ): CertificateDraftIndexItem[] => {
  ...
  const indexesAreEqual = (
    a: CertificateDraftIndexItem[],
    b: CertificateDraftIndexItem[]
  ): boolean => {
    const stable = (items: CertificateDraftIndexItem[]) =>
      JSON.stringify(
  ...
  const reconcileQueueIndexes = (
    ...
    const archived = mergeDraftIndexes(archivedItems).filter(
      (it) => !activeRcSet.has(normalizeRc(it.rc))
    );
    return { active, archived };
  };

  const queueItemMatchesSearch = (
    item: CertificateDraftIndexItem,
    searchValue: string
  ): boolean => {
  ...
  const readStorageTextByCandidates = async (
    candidates: string[]
  ...
  const loadIssuedCertificatesIndex = async (
    organizationId: string
  ...
  const saveIssuedCertificatesIndex = async (
    organizationId: string,
    items: IssuedCertificateRecord[]
  ) => {
    ...
    const { error } = await supabase.storage
      .from("work_photos")
      .upload(getIssuedIndexPath(organizationId), blob, {
        upsert: true,
  ...
  const loadArchivedDraftIndex = async (
    organizationId: string
  ): Promise<CertificateDraftIndexItem[]> => {
    const resolved = await readStorageTextByCandidates([
  ...
  const recoverIndexesFromStoredDrafts = async (
    ...
    active: CertificateDraftIndexItem[];
    archived: CertificateDraftIndexItem[];
    ...
    const archived = mergeDraftIndexes(archivedRecovered).filter(
      (it) => !active.some((activeItem) => normalizeRc(activeItem.rc) === normalizeRc(it.rc))
    );
  ...
  const loadDraftPayload = async (
    organizationId: string,
    rc: string
  ): Promise<CertificateDraftPayload | null> => {
  ...
  const saveIndexFile = async (indexPath: string, items: CertificateDraftIndexItem[]) => {
    const blob = new Blob([JSON.stringify(sortDrafts(items), null, 2)], {
  ...
  const saveArchivedDraftIndex = async (
    organizationId: string,
    items: CertificateDraftIndexItem[]
  ) => {
  ...
  const archivarCompletados = async (options?: {
    onlyRc?: string;
    skipConfirm?: boolean;
  }): Promise<boolean> => {
    if (!supabase) return false;
    try {
      ...
      const completedItems = active.filter(
        (d) =>
          d.status === "completado" && (!onlyRcNormalized || normalizeRc(d.rc) === onlyRcNormalized)
      );
  ...
  const repararArchivadosEnvolvente = async () => {
    ...
    try {
      ...
      const updatedMeta = new Map<
        string,
        { updatedAt: string; clienteNombre: string; clienteDni: string; status: CertDraftStatus }
      >();
      ...
      for (let i = 0; i < archived.length; i += 1) {
        ...
        const opacosList = Array.isArray(payload.elementosOpacosList)
          ? payload.elementosOpacosList
          : [];
        ...
        const zonaValida =
          payload.zonaKey && VALORES_G[payload.zonaKey] !== undefined ? payload.zonaKey : "D3";
        ...
        if (changed) {
          ...
          const blob = new Blob([JSON.stringify(repairedPayload, null, 2)], {
            type: "application/json",
          });
          const { error: uploadError } = await supabase.storage
            .from("work_photos")
            .upload(getDraftPath(organizationId, rc), blob, {
              upsert: true,
              contentType: "application/json",
            });
    ...
    try {
      ...
      const warningText =
        exportIssues.length > 0
          ...
          : "";
      setDraftMsg(
        `Backup ZIP exportado (${draftsForExport.length}/${draftQueue.length}).${warningText}`
      );
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".zip")) {
        ...
        if (backupFile) {
          ...
          const draftFiles = Object.values(zip.files).filter(
            (entry) => !entry.dir && /^drafts\/.+\.json$/i.test(entry.name)
          );
      ...
      for (let i = 0; i < draftsToRestore.length; i += 1) {
        ...
        const newItem: CertificateDraftIndexItem = {
          ...
          clienteNombre:
            payloadToStore.clienteNombre ||
            [
              payloadToStore.clienteFirstName,
              payloadToStore.clienteMiddleName,
              payloadToStore.clienteLastName1,
              payloadToStore.clienteLastName2,
            ]
              .filter(Boolean)
              .join(" "),
      ...
      const archivedAfterImport = archivedBeforeImport.filter(
        (it) => !importedRcSet.has(normalizeRc(it.rc))
      );
  ...
  const refreshDraftQueue = async () => {
    ...
    try {
      ...
      const organizationCandidates =
        await collectDraftOrganizationCandidates(primaryOrganizationId);
      ...
      if (!indexesAreEqual(selectedQueue.archivedItemsRaw, selectedQueue.archived)) {
        persistTasks.push(
          saveArchivedDraftIndex(selectedQueue.organizationId, selectedQueue.archived)
        );
      }
  ...
  const applyDraftPayload = (payload: CertificateDraftPayload) => {
    ...
    setCapas(
      Array.isArray(payload.capas) && payload.capas.length > 0 ? payload.capas : cloneInitialCapas()
    );
    ...
    setVentilationLocked(
      typeof payload.ventilationLocked === "boolean" ? payload.ventilationLocked : true
    );
    ...
    setLastCalculatedFingerprint(
      payload.resultado
        ? buildCalcFingerprint({
            capas:
              Array.isArray(payload.capas) && payload.capas.length > 0
                ? payload.capas
                : cloneInitialCapas(),
  ...
  const saveCurrentDraft = async (
    ...
    try {
      ...
      if (!options?.suppressSuccessMessage) {
        ...
        const extraMsg = [mvpSyncNote, mvpSyncWarning ? `Aviso: ${mvpSyncWarning}` : null]
          .filter(Boolean)
          .join(" ");
  ...
  const restaurarTodosArchivados = async () => {
    ...
    if (!confirm(`¿Restaurar ${archivedQueue.length} expediente(s) archivado(s) a la cola activa?`))
      return;
  ...
  const completarYCargarSiguiente = async () => {
    ...
    try {
      ...
      if (!payload) {
        setDraftMsg(
          `Expediente ${rcNormalized} completado. El siguiente (${next.rc}) no se pudo cargar automáticamente.`
        );
        return;
      }
      ...
      applyDraftPayload(payload);
      setDraftMsg(
        `Expediente ${rcNormalized} completado. Cargado siguiente pendiente: ${next.rc}.`
      );
    } catch (error: any) {
  ...
  const copiarDatosClavePDF = async () => {
    ...
    const porcentajeEnvolvente =
      supEnvolvente > 0 ? roundTo((supActuacion / supEnvolvente) * 100, 2) : 0;
  ...
  const prepararSiguienteLote = async () => {
    ...
    try {
      ...
      const archivedUpdated =
        completedActive.length > 0 ? mergeDraftIndexes(archived, completedActive) : archived;
      ...
      const archivedMsg =
        completedActive.length > 0 ? `${completedActive.length} completado(s) archivado(s). ` : "";
      setDraftMsg(
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] p-6 gap-5 animate-in fade-in duration-500 overflow-y-auto">
      ...
      <Card className="bg-slate-900/40 border-slate-800">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-slate-500">
            Guarda por Referencia Catastral, retoma expedientes en segundos y marca completados sin
            perder cálculos ni capturas.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <div
              className={`md:col-span-2 grid-cols-3 gap-2 items-end transition-all duration-300 ${isExperto ? "grid" : "hidden"}`}
            >
              <button
          ...
          <div
            className={`flex flex-wrap items-center gap-2 text-xs transition-all duration-300 ${isExperto ? "block" : "hidden"}`}
          >
            <span className="px-2 py-1 rounded border border-slate-700 text-slate-300 bg-slate-900/40">
              Total: {queueTotal}
            </span>
            <span className="px-2 py-1 rounded border border-amber-700/40 text-amber-300 bg-amber-900/20">
              Pendientes: {queuePending}
            </span>
            <span className="px-2 py-1 rounded border border-emerald-700/40 text-emerald-300 bg-emerald-900/20">
              Completados: {queueCompleted}
            </span>
            <span className="px-2 py-1 rounded border border-violet-700/40 text-violet-300 bg-violet-900/20">
              Archivados: {archivedTotal}
            </span>
            <span className="px-2 py-1 rounded border border-cyan-700/40 text-cyan-300 bg-cyan-900/20">
              Emitidos cloud: {issuedCertificatesCount}
            </span>
            {isExpedienteMvpSyncEnabled() && (
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="relative">
              ...
              <GestionLotesSheet open={isLotesSheetOpen} onOpenChange={setIsLotesSheetOpen}>
                <button
                  onClick={() => {
                    void copiarDatosClavePDF();
                    setIsLotesSheetOpen(false);
                  }}
                ...
                <button
                  onClick={() => {
                    void exportarHistorialEmitidosCSV();
                    setIsLotesSheetOpen(false);
                  }}
            ...
            <div className="rounded-md border border-cyan-700/40 bg-cyan-900/10 px-3 py-2 space-y-2">
              ...
              <div className="flex items-center justify-between gap-2">
                <p className="text-[11px] text-cyan-100/90 truncate">
                  {batchProgress.detail || "Procesando..."}
                </p>
                <button
          ...
          {pendingConflictResolution && (
            <div className="text-xs text-rose-200 bg-rose-950/30 border border-rose-700/40 px-3 py-3 rounded-md space-y-2">
              <p className="font-semibold">Conflicto SQL MVP en {pendingConflictResolution.rc}</p>
              ...
              <p className="text-rose-100/80">
                Campos distintos:{" "}
                {pendingConflictResolution.context.diffKeys.length > 0
                  ? pendingConflictResolution.context.diffKeys.slice(0, 8).join(", ")
                  : "sin detalle disponible"}
                .
              </p>
              <p className="text-rose-100/70">
                Última actualización remota:{" "}
                {pendingConflictResolution.context.remoteUpdatedAt
          ...
          <div
            className={`grid grid-cols-1 ${showArchivedQueuePanel ? "md:grid-cols-2" : ""} gap-2 transition-all duration-300 ${isExperto ? "block" : "hidden"}`}
          >
          ...
          {!showArchivedQueuePanel && archivedTotal > 0 && (
            <p className="text-[11px] text-violet-200/70">
              Archivados ocultos para no mezclar el lote actual. Usa "Ver archivados" cuando
              necesites recuperar alguno.
            </p>
          )}

          <div
            className={`rounded-md border border-slate-800 bg-slate-950/30 max-h-52 overflow-y-auto transition-all duration-300 ${isExperto ? "block" : "hidden"}`}
          >
            {draftLoading ? (
              <div className="px-3 py-3 text-xs text-slate-400">
                Cargando cola de certificados...
              </div>
            ) : draftQueue.length === 0 ? (
              <div className="px-3 py-3 text-xs text-slate-500">
                Sin expedientes en cola. Guarda el primero para iniciar el lote.
              </div>
            ) : filteredDraftQueue.length === 0 ? (
              <div className="px-3 py-3 text-xs text-slate-500">
                Sin coincidencias en cola activa para "{queueSearch}".
              </div>
            ) : (
              <div className="divide-y divide-slate-800">
                {filteredDraftQueue.map((item) => (
                  <div key={item.rc} className="px-3 py-2 flex items-center gap-2">
                    <button onClick={() => loadDraft(item)} className="text-left flex-1 min-w-0">
          ...
          {showArchivedQueuePanel && (
            <div className="rounded-md border border-violet-900/40 bg-violet-950/20 max-h-44 overflow-y-auto">
              {archivedQueue.length === 0 ? (
                <div className="px-3 py-3 text-xs text-violet-200/70">
                  Sin expedientes archivados.
                </div>
              ) : filteredArchivedQueue.length === 0 ? (
                <div className="px-3 py-3 text-xs text-violet-200/70">
                  Sin coincidencias en archivados para "{archivedSearch}".
                </div>
              ) : (
      ...
      <Card className="bg-slate-900/40 border-slate-800">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-slate-500">
            Importa XML para rellenar superficies/zona y usa DNI para autocompletar cliente desde
            base de datos.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {catastroVerificationBanner && (
            <div
              className={`text-xs px-3 py-2 rounded-md border ${
                catastroVerificationBanner.tone === "warning"
                  ? "text-amber-300 bg-amber-500/10 border-amber-500/30"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
            <div>
              ...
              <Input
                value={clienteFirstName}
                onChange={(e) => setClienteFirstName(e.target.value)}
                placeholder="Juan"
            ...
            <div>
              ...
              <Input
                value={clienteMiddleName}
                onChange={(e) => setClienteMiddleName(e.target.value)}
                placeholder="Carlos"
                className="h-9 bg-slate-900/50 border-slate-700 text-slate-200"
              />
            </div>
            <div>
              ...
              <Input
                value={clienteLastName1}
                onChange={(e) => setClienteLastName1(e.target.value)}
                placeholder="García"
                className="h-9 bg-slate-900/50 border-slate-700 text-slate-200"
              />
            </div>
            <div>
              ...
              <Input
                value={clienteLastName2}
                onChange={(e) => setClienteLastName2(e.target.value)}
                placeholder="López"
                className="h-9 bg-slate-900/50 border-slate-700 text-slate-200"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={
                    savingCliente ||
                    !clienteFirstName.trim() ||
                    !clienteLastName1.trim() ||
                    !clienteDni.trim()
                  }
              ...
              <div className="rounded-md border border-indigo-500/20 bg-indigo-500/5 p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      <RefreshCcw
                        className={`h-3 w-3 transition-transform duration-300 ${dniFlipped ? "rotate-180" : ""}`}
                      />
                      Voltear
                    </button>
                  </div>
                ...
                {activeData ? (
                  ...
                  <div className="w-full h-20 rounded border border-dashed border-indigo-700/30 flex items-center justify-center text-[10px] text-slate-500">
                    Sin captura de {activeLabel.toLowerCase()} — cárgala en el panel de capturas
                    inferior
                  </div>
                )}
              </div>
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-lg border border-slate-800 bg-slate-900/30 p-3">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className="text-[10px] uppercase font-bold text-slate-400">
                    Materiales detectados en capas:
                  </span>
                  <span
                    className={`px-2 py-1 rounded border text-[10px] ${materialFlags.hormigon ? "border-emerald-600/40 text-emerald-300 bg-emerald-900/20" : "border-slate-700 text-slate-500"}`}
                  >
                    Hormigón
                  </span>
                  <span
                    className={`px-2 py-1 rounded border text-[10px] ${materialFlags.yeso ? "border-emerald-600/40 text-emerald-300 bg-emerald-900/20" : "border-slate-700 text-slate-500"}`}
                  >
                    Yeso
                  </span>
                  <span
                    className={`px-2 py-1 rounded border text-[10px] ${materialFlags.madera ? "border-emerald-600/40 text-emerald-300 bg-emerald-900/20" : "border-slate-700 text-slate-500"}`}
                  >
                    Madera
                  </span>
                </div>
                <p className="text-[11px] text-slate-500">
                  El bloque de capas ahora empieza vacío. Añade capas y trabaja rápido con R
                  directo; espesor/λ quedan como detalle opcional.
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  {materialSupportSlots.map((slot) => {
                    ...
                    return (
                      <div
                        key={slot.key}
                        className="rounded-md border border-slate-800 p-2 bg-slate-900/50"
                      >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      ...
                      <button
                        onClick={() =>
                          setShowAdvancedByLayer((prev) => ({ ...prev, [i]: !prev[i] }))
                        }
                      ...
                      <div className="mt-2 space-y-1">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          <select
                            ...
                            value={methodFilter}
                            onChange={(e) =>
                              setFiltroMetodo((prev) => ({ ...prev, [i]: e.target.value }))
                            }
                          >
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setMaterialSearchByLayer((prev) => ({ ...prev, [i]: e.target.value }))
                            }
                        ...
                        >
                          ...
                          {materialesFiltrados.map((m) => (
                            <option key={m.id} value={m.id}>
                              ...
                              {m.application_method ? ` [${m.application_method}]` : ""}
                            </option>
                          ))}
                        </select>
                      </div>
              ...
              <div className="rounded-md border border-slate-800 bg-slate-900/20 p-2 space-y-2">
                <p className="text-[10px] uppercase font-bold text-slate-400">
                  Plantillas rápidas para tu lote
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  <button
          ...
          <Card className="bg-slate-900/40 border-slate-800">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  <label className="text-[10px] text-blue-400 uppercase font-bold">
                    Superficie Partición (m²)
                  </label>
                ...
                <div>
                  ...
                  <p className="text-[9px] text-slate-600 mb-1">
                    Sin cubierta; si CE3X trae huecos embebidos en S, revisa el desglose.
                  </p>
                  <Input
                    type="number"
                    step="0.01"
          ...
          <Card className="bg-slate-900/40 border-slate-800">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-slate-500">
                Tabla 7 CTE DB-HE. Define cómo estaba el cerramiento ANTES y DESPUÉS de tu
                intervención.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
                <div>
                  <p className="text-xs text-slate-300 font-semibold">
                    Ventilación anclada (Sincronizada)
                  </p>
                  <p className="text-[10px] text-slate-500">
                    Al cambiar el caso ANTES, el caso DESPUÉS se ajustará igual.
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                <div>
                  ...
                  <p className="text-[9px] text-slate-600">
                    Up a 2 decimales para cuadrar con valores del programa CE3X
                  </p>
                </div>
                <button
              ...
              <div className="pt-2 border-t border-slate-800 space-y-2">
                <p className="text-[10px] text-purple-300 uppercase font-bold">
                  Ajuste CE3X (opcional)
                </p>
                <p className="text-[10px] text-slate-500">
                  Si CE3X te da valores exactos de Ui/Uf, puedes forzarlos aqui para copiar el
                  informe con esa precision.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div>
              ...
              <div className="flex items-center justify-between gap-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs transition-colors ${!isCloudCalculation ? "text-slate-200 font-bold" : "text-slate-500"}`}
                  >
                    Local
                  </span>
                  <button
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isCloudCalculation ? "translate-x-6" : "translate-x-1"}`}
                    />
                  </button>
                  <span
                    className={`text-xs transition-colors ${isCloudCalculation ? "text-amber-500 font-bold" : "text-slate-500"}`}
                  >
                    On-Cloud
                  </span>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            <Card className="bg-slate-900/40 border-slate-800">
              <CardHeader className="pb-2">
                <CardTitle className="text-[11px] text-slate-400 uppercase font-bold flex items-center justify-between">
                  <span className="flex items-center gap-1">
                    <ZoomIn className="h-3 w-3" /> Resumen CEE Inicial
                  </span>
              ...
              <CardContent className="space-y-3 p-3">
                {capturas.cee_inicial?.dataUrl && (
                  <div
                    className="w-full h-40 rounded overflow-hidden relative border border-slate-700 bg-black cursor-zoom-in group"
                    onClick={() => openCapturaPreview("cee_inicial", "Etiqueta CEE Inicial")}
                  >
                    <img
                      src={capturas.cee_inicial.dataUrl}
                      className="w-full h-full object-contain opacity-80 group-hover:opacity-100 transition-opacity"
                    />
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 transition-opacity text-white text-xs font-bold">
                      Ver Completa
                    </div>
                  </div>
                )}
                <div className="space-y-2 text-xs text-slate-300 bg-slate-900/50 rounded-lg border border-slate-800 overflow-hidden">
                  <button
                  ...
                  >
                    ...
                    <span className="text-[9px]">
                      {desgloseOpen ? "▲ Minimizar" : "▼ Expandir"}
                    </span>
                  </button>
                  {desgloseOpen && (
                            ...
                            <div className="space-y-1">
                              <div className="flex justify-between items-center bg-slate-800/80 p-1 rounded">
                                <span className="text-[11px] text-slate-300 font-semibold px-1">
                                  Cerramientos Opacos (CE3X)
                                </span>
                                <span className="font-mono text-[11px] font-bold text-slate-300 mr-1">
                                  {Number(supOpacos).toFixed(2)} m²
                                </span>
                              </div>
                                  ...
                                  <tbody className="divide-y divide-slate-800/80 bg-slate-900/30">
                                    {elementosOpacosList.map((el, i) => (
                                      ...
                                      >
                                        ...
                                        <td
                                          className="py-1 px-2 truncate max-w-[80px] text-slate-500"
                                          title={el.tipo}
                                        >
                                          {el.tipo}
                                        </td>
                                        <td className="py-1 px-2 text-right font-mono">
                                          {el.superficie.toFixed(2)}
                                        </td>
                                        <td className="py-1 px-2 text-right font-mono text-slate-500">
                                          {el.transmitancia ? el.transmitancia.toFixed(2) : "—"}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
                                ...
                                <table className="w-full text-[10px] text-left">
                                  ...
                                  <tbody className="divide-y divide-slate-800/80 bg-slate-900/30">
                                    {elementosHuecosList.map((el, i) => (
                                      ...
                                      >
                                        <td
                                          className="py-1 px-2 truncate max-w-[140px]"
                                          title={el.nombre}
                                        >
                                          {el.nombre}
                                        </td>
            ...
            <>
              ...
              <Card className="bg-slate-900/40 border-slate-800">
                ...
                <CardContent className="space-y-3 text-sm">
                  <ResultRow
                    label="ΣR materiales (i)"
                    value={`${resultado.r_mat_inicial.toFixed(3)} m²K/W`}
                  />
                  <ResultRow
                    label="ΣR materiales (f)"
                    value={`${resultado.r_mat_final.toFixed(3)} m²K/W`}
                    color="emerald"
                  />
                  <div className="border-t border-slate-800 my-2" />
                  <ResultRow
                    label="RT inicial"
                    value={`${resultado.rt_inicial.toFixed(3)} m²K/W`}
                  />
                  <ResultRow
                    label="RT final"
                    value={`${resultado.rt_final.toFixed(3)} m²K/W`}
                    color="emerald"
                  />
                  <div className="border-t border-slate-800 my-2" />
                  <ResultRow
                    label={`Up inicial (${modoCE3X ? "2" : "3"} dec)`}
                    value={`${resultado.up_inicial.toFixed(modoCE3X ? 2 : 3)} W/m²K`}
                  />
                  <ResultRow
                    label={`Up final (${modoCE3X ? "2" : "3"} dec)`}
                    value={`${resultado.up_final.toFixed(modoCE3X ? 2 : 3)} W/m²K`}
                    color="emerald"
                  />
                  <div className="border-t border-slate-800 my-2" />
                  <ResultRow label="Factor b (antes)" value={resultado.b_inicial.toFixed(2)} />
                  <ResultRow
                    label="Factor b (después)"
                    value={resultado.b_final.toFixed(2)}
                    color="emerald"
                  />
                  <div className="border-t border-slate-800 my-2" />
                  ...
                  <ResultRow
                    label="Uf (después)"
                    value={`${resultado.uf_final.toFixed(2)} W/m²K`}
                    color="emerald"
                  />
                  <ResultRow
                    label="ΔU"
                    value={`${(resultado.ui_final - resultado.uf_final).toFixed(2)} W/m²K`}
                    color="orange"
                  />
                  <div className="border-t border-slate-800 my-2" />
                  <ResultRow
                    label="% Envolvente"
                    value={`${resultado.pct_envolvente.toFixed(2)}%`}
                  />
              ...
              {resultadoDesactualizado && (
                <Card className="bg-amber-950/40 border-amber-500/40">
                  <CardContent className="p-3 text-xs text-amber-200 flex items-center gap-2">
                    ...
                    <span>
                      Resultado desactualizado: has cambiado datos después del último cálculo. Pulsa
                      "Calcular Ahorro Energético" antes de copiar o generar documentos.
                    </span>
                  </CardContent>
                </Card>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
                ...
                <button
                  ...
                  title={
                    outputActionsDisabled
                      ? outputActionDisabledTitle
                      : "Generar certificado en Word (DOCX)"
                  }
                >
          ...
          {certStatus === "completado" && (
            <CertificadoSuccessState
              ...
              onDescargarPDF={() => {
                void generarCertificadoIntelliaPDF();
              }}
      ...
      <CertificadoCapturasPanelControlado capturas={capturas} onCapturasChange={setCapturas} />
...
function HoverZoomImage({
  ...
  const handleMove = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    if (imageEl && imageEl.naturalWidth > 0 && imageEl.naturalHeight > 0) {
      const scale = Math.min(
        rect.width / imageEl.naturalWidth,
        rect.height / imageEl.naturalHeight
      );
  ...
  const panelInlineStyle =
    panelPlacement === "inside" ? { maxWidth: "calc(100vw - 16px)" } : undefined;
