...
function diffKeys(
    ...
    const keys = new Set<string>([
        ...Object.keys(localDatos),
        ...Object.keys(remoteDatos),
    ]);
...
async function fetchRemoteExpedienteSnapshot(
    ...
    const { data, error } = write.expedienteId
        ? await queryById()
        : await queryByRc();
...
export async function resolveConflict(
    ...
    const isNetworkRetryWindow = write.retryCount > 0
        && write.retryCount <= 3
        && deltaMs < RETRY_LOCAL_WINS_WINDOW_MS;
...
async function handleVersionConflict(
    ...
    const localUpdatedAt = Number.isFinite(write.localUpdatedAt)
        ? write.localUpdatedAt
        : Date.now();
    ...
    await markOfflineExpedienteNeedsUserChoice(
        write.rc,
        toStoredConflictContext(context),
        {
            expedienteId: remote.id,
            versionToken: remote.versionToken,
        },
...
export async function flushOfflineExpedienteQueueNow(
    ...
    for (const write of writes) {
        ...
        await markOfflineExpedienteRetry(
            rc,
            result.errorMessage ?? "SYNC_FAILED",
            {
