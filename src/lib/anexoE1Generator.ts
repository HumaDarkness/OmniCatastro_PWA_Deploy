...
export function generarPDFAnexoE1(capas: CapaMaterial[], resultado: ResultadoTermico) {
    ...
    yPos += 6;
    doc.text(`Transmitancia Térmica Parcial Inicial (U_p): ${resultado.up_inicial.toFixed(3)} W/m²K`, 14, yPos);
    yPos += 6;
    ...
    yPos += 6;
    doc.text(`Transmitancia Térmica Parcial Final (U_p): ${resultado.up_final.toFixed(3)} W/m²K`, 14, yPos);
    yPos += 6;
    ...
    doc.text(`Transmitancia Térmica Final Mejorada (U_f): ${resultado.uf_final.toFixed(3)} W/m²K`, 14, yPos);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Reducción de Transmitancia (ΔU): ${(resultado.ui_final - resultado.uf_final).toFixed(3)} W/m²K`, 14, yPos);
    yPos += 6;
    doc.setFont("helvetica", "bold");
