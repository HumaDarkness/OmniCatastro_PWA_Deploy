...
describe('thermalCalculator - Coeficiente b', () => {
    ...
    it('Fixture 2: NADA aislado, Estanco', () => {
        expect(getB(0.82, 'nada_aislado', 'estanco')).toBe(0.70);
    });
    ...
    it('Fixture 4: Partición aislada, Estanco (ratio alto)', () => {
        expect(getB(1.50, 'particion_aislada', 'estanco')).toBe(0.89);
    });

    it('Fixture 5: NADA aislado, Ventilado (ratio límite)', () => {
        expect(getB(3.50, 'nada_aislado', 'ventilado')).toBe(0.57);
    });
    ...
    it('Edge Case: ratio exacto 5.00', () => {
        expect(getB(5.00, 'particion_aislada', 'estanco')).toBe(0.81);
    });

    it('Edge Case: ratio mayor a 5.00', () => {
        expect(getB(10.00, 'particion_aislada', 'estanco')).toBe(0.81);
    });
...
describe('thermalCalculator - Caso Real Madrid D3', () => {
    it('Coincidencia con Python (AE ≈ 28,870 kWh)', () => {
        const res = calcularAhorroCAE({
            capas: [
                { nombre: "Hormigón", espesor: 0.10, lambda_val: 2.5, r_valor: 0, es_nueva: false },
        ...
        // b_inicial
        expect(res.b_inicial).toBe(0.70);
        // b_final
