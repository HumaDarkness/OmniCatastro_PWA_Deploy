...
export function ClientesView() {
  ...
  async function listDraftRcCandidates(folderPath: string): Promise<string[]> {
    ...
    while (true) {
      ...
      for (const entry of data) {
        ...
        if (
          rawName === "_index.json" ||
          rawName === "_archived_index.json" ||
          rawName === "_import_audit.json"
        ) {
          continue;
        }
  ...
  async function loadDniBlobsFromDrafts(
    ...
    const processRcCandidates = async (rcCandidates: string[]) => {
      for (let i = 0; i < rcCandidates.length; i += 1) {
        ...
        const captures =
          (parsed.capturas && typeof parsed.capturas === "object"
  ...
  async function syncCloudClientsToLocal(options?: { silent?: boolean }) {
    ...
    try {
      ...
      let query = supabase.from("clients").select("*").limit(500);
      ...
      if (error && /organization_id/i.test(error.message || "")) {
        const retry = await supabase.from("clients").select("*").limit(500);
        data = retry.data;
        error = retry.error;
      }
      ...
      for (const cloudClient of cloudClients) {
        const nif =
          pickFirstString(cloudClient, ["dni", "nif", "document_id", "documento"])?.toUpperCase() ||
          "";
        if (!nif) continue;
        const nifKey = normalizeNifKey(nif);
        const localClient = localByNifKey.get(nifKey);
        ...
        const telefono = pickFirstString(cloudClient, [
          "phone",
          "telefono",
          "phone_number",
          "mobile",
        ...
        const frontCandidates: Array<string | undefined> = [dniFrontPath, indexedPaths?.front];
        const backCandidates: Array<string | undefined> = [dniBackPath, indexedPaths?.back];
        ...
        const [dniBlobFront, dniBlobBack] = await Promise.all([
          existingFront
            ...
            : downloadFirstAvailableBlob(frontCandidates, {
                maxAttempts: shouldUseHeuristicPaths ? 3 : 1,
              }),
          existingBack
            ? Promise.resolve(existingBack)
            : downloadFirstAvailableBlob(backCandidates, {
                maxAttempts: shouldUseHeuristicPaths ? 3 : 1,
              }),
        ]);
        ...
        const shouldPersist =
          metadataChanged || hasNewFront || hasNewBack || !localClient?.syncedAt;
        ...
        if (!options?.silent && processedCount % 50 === 0) {
          setSyncMsg(
            `Sincronizando clientes... ${processedCount}/${cloudClients.length} (actualizados: ${importedCount}).`
          );
        }
      }
      ...
      setSyncMsg(
        `Clientes revisados: ${processedCount}. Actualizados local: ${importedCount}. Sin cambios: ${unchangedCount}. Con imágenes DNI: ${withDniImages}.${imageHint}${listedHint}`
      );
    } catch (error: any) {
      setSyncMsg(
        `No se pudo sincronizar clientes desde la nube: ${error?.message ?? "Error desconocido"}.`
      );
    } finally {
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((client) => {
      const searchable = normalizeClientSearch(
        [client.nombre, client.apellidos, client.nif, client.email ?? ""].join(" ")
      );
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <input
                    ...
                    ref={frontInputRef}
                    onChange={(e) =>
                      e.target.files?.[0] && handleFileChange(e.target.files[0], "front")
                    }
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <input
                    ...
                    ref={backInputRef}
                    onChange={(e) =>
                      e.target.files?.[0] && handleFileChange(e.target.files[0], "back")
                    }
                  />
                </div>
              </div>
            ...
            <div className="max-w-md mx-auto space-y-6">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  Cambios se guardan en el navegador y sincronizan en background.
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingClient((prev) => ({ ...prev!, nombre: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingClient((prev) => ({ ...prev!, apellidos: e.target.value }))
                    }
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-4 py-2.5 text-white focus:border-indigo-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingClient((prev) => ({ ...prev!, nif: e.target.value }))
                    }
          ...
          {filteredClients.map((client) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-4 text-xs text-slate-500 border-t border-slate-800/50 pt-3">
                <span
                  className={`flex items-center gap-1 ${!client.syncedAt ? "text-amber-500" : "text-emerald-500"}`}
                >
                  <div
                    className={`w-2 h-2 rounded-full ${!client.syncedAt ? "bg-amber-500" : "bg-emerald-500"}`}
                  ></div>
