...
export function CertificadoCapturasPanel({
  capturas: externalCapturas,
  setCapturas: externalSetCapturas,
}: Props = {}) {
  const [internalCapturas, setInternalCapturas] = useState<CapturasState>(
  ...
  const totalCargadas = useMemo(() => Object.values(capturas).filter(Boolean).length, [capturas]);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          {SLOT_CONFIG.map((slot) => {
            ...
            return (
              <div
                key={slot.key}
                className="border border-slate-800 rounded-lg p-3 bg-slate-900/30 space-y-2"
              >
                <div>
                ...
                {data ? (
                  <img src={data.dataUrl} alt={slot.label} className={previewClass} />
                ) : (
                  <div
                    className={`w-full ${previewHeightClass} rounded-md border border-dashed border-slate-700 bg-slate-900/40 flex items-center justify-center text-[11px] text-slate-600`}
                  >
                    Sin imagen
                  </div>
                )}
