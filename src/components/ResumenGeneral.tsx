        ...
        <div className="w-full h-full overflow-y-auto p-4 md:p-6 lg:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">
                        Métricas de rendimiento y monitorización operativa
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <MetricCard 
                    ...
                    icon={<RefreshCcw className={`w-6 h-6 ${metrics.syncPendientes > 0 ? "text-rose-400 animate-spin" : "text-slate-400"}`} />}
                    ...
                    accentColor={metrics.syncPendientes > 0 ? "bg-rose-400 shadow-[0_0_10px_rgba(251,113,133,0.6)]" : "bg-slate-500"}
                />
...
function MetricCard({ title, value, icon, colorClass, accentColor }: { title: string, value: string, icon: React.ReactNode, colorClass: string, accentColor: string }) {
    return (
        <div className={`relative overflow-hidden bg-gradient-to-br backdrop-blur-xl border rounded-2xl p-6 ${colorClass} transition-transform hover:scale-[1.02] duration-300`}>
            <div className={`absolute top-0 left-0 w-1 h-full ${accentColor}`}></div>
            <div className="flex justify-between items-start mb-4">
                <div className="p-2 rounded-xl bg-slate-900/50 shadow-inner">
                    {icon}
                </div>
            </div>
