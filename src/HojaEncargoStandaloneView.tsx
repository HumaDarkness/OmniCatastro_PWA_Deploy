...
export function HojaEncargoStandaloneView() {
  ...
  const [tecnico, setTecnico] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { nombre: "", nif: "", empresa: "", direccion: "", ciudad: "", cp: "" };
  });
  ...
  // Alerts
  const [uxMessage, setUxMessage] = useState<{ type: "error" | "success"; text: string } | null>(
    null
  );
  ...
  useEffect(() => {
    if (tecnico.nombre) {
      localStorage.setItem(
        "tecnicoProfile",
        JSON.stringify({ ...tecnico, firmaBase64: techFirmaUrl || undefined })
      );
    }
  ...
  async function loadClientsList() {
    const { data } = await supabase
      .from("clients")
      .select("id, first_name, last_name_1, last_name_2, dni, address")
    ...
    try {
      ...
      if (techSigCanvasRef.current && !techSigCanvasRef.current.isEmpty()) {
        const blob = await fetch(
          techSigCanvasRef.current.getTrimmedCanvas().toDataURL("image/png")
        ).then((r) => r.blob());
      ...
      if (pdfBlob) {
        saveAs(
          pdfBlob,
          `Hoja_Encargo_${propietarioPayload.nombre.replace(/ /g, "_")}_${new Date().getFullYear()}.pdf`
        );
        setUxMessage({
          type: "success",
          text: "Hoja de Encargo generada y descargada localmente.",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#060612] relative">
        ...
        <div className="flex items-center gap-4 text-indigo-400">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1 flex items-center gap-2">
              Generador Autónomo{" "}
              <span title="Generación local PDF">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        {uxMessage && (
          <div
            className={`mb-6 p-4 rounded-xl border flex items-center gap-3 ${uxMessage.type === "error" ? "bg-rose-500/10 border-rose-500/20 text-rose-300" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-300"} animate-in fade-in slide-in-from-top-2`}
          >
            {uxMessage.type === "error" ? (
          ...
          <div className="xl:col-span-7 space-y-6">
            ...
            <div className="bg-[#0a0a1a] border border-slate-800/60 rounded-xl p-5 md:p-6 shadow-sm">
              <h2 className="text-sm font-bold text-slate-200 mb-4 flex items-center gap-2 border-b border-slate-800/50 pb-3">
                <MapPin className="w-4 h-4 text-indigo-400" /> Identificación del Inmueble
                (Catastro)
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="md:col-span-4">
                  ...
                  <input
                    ...
                    onChange={(e) => setInmueble({ ...inmueble, tipoVia: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-8">
                  ...
                  <input
                    type="text"
                    value={inmueble.nombreVia}
                    onChange={(e) => setInmueble({ ...inmueble, nombreVia: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setInmueble({ ...inmueble, numero: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={inmueble.bloque}
                    onChange={(e) => setInmueble({ ...inmueble, bloque: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={inmueble.escalera}
                    onChange={(e) => setInmueble({ ...inmueble, escalera: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setInmueble({ ...inmueble, planta: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setInmueble({ ...inmueble, puerta: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                ...
                <div className="md:col-span-3">
                  ...
                  <input
                    type="text"
                    value={inmueble.cp}
                    onChange={(e) => setInmueble({ ...inmueble, cp: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-5">
                  ...
                  <input
                    ...
                    onChange={(e) => setInmueble({ ...inmueble, municipio: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div className="md:col-span-4">
                  ...
                  <input
                    type="text"
                    value={inmueble.provincia}
                    onChange={(e) => setInmueble({ ...inmueble, provincia: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
              </div>
            ...
            <div className="bg-[#0a0a1a] border border-slate-800/60 rounded-xl p-5 md:p-6 shadow-sm ring-1 ring-blue-500/10 relative overflow-hidden">
              ...
              <h2 className="text-sm font-bold text-slate-200 mb-4 flex items-center justify-between border-b border-slate-800/50 pb-3">
                <span className="flex items-center gap-2">
                  <Search className="w-4 h-4 text-blue-400" /> Cliente / Contratante
                </span>
              ...
              {!showQuickAdd ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in">
                  <div>
                    ...
                    >
                      <option value="" className="bg-[#0f0f23]">
                        -- Seleccionar --
                      </option>
                      {clientsList.map((c) => (
                      ...
                      <option value="NEW" className="bg-[#0f0f23] text-indigo-400 font-bold">
                        + Borrador Manual (No guardar)
                      </option>
                    </select>
                  </div>
                  <div>
                  ...
                  {selectedClient === "NEW" && (
                    <div className="col-span-2 p-3 border border-dashed border-slate-700 rounded-lg bg-slate-900/30">
                      <p className="text-xs text-slate-400 mb-2">
                        Has seleccionado crear un borrador rápido que NO se guardará en la base de
                        datos de Clientes. Si prefieres guardarlo, haz click en "Quick Add" arriba.
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          className="bg-black/20 border border-slate-800 rounded px-2 py-1.5 text-sm text-white"
                        />
                        <input
                          type="text"
                          placeholder="DNI/NIF"
                          value={quickClient.dni}
                          onChange={(e) => setQuickClient({ ...quickClient, dni: e.target.value })}
                          className="bg-black/20 border border-slate-800 rounded px-2 py-1.5 text-sm text-white"
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-4 animate-in slide-in-from-top-2">
                  <div className="p-3 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/20 rounded-xl">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-3 bg-purple-600/80 hover:bg-purple-500 text-white rounded-lg text-xs font-medium transition-colors"
                      >
                        Extraer
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] text-slate-500 mb-1 uppercase">
                        Nombre
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full bg-black/20 border border-slate-700 rounded px-3 py-1.5 text-slate-200 text-sm outline-none focus:border-blue-500"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-slate-500 mb-1 uppercase">
                      ...
                      <input
                        type="text"
                        value={quickClient.apellido1}
                        onChange={(e) =>
                          setQuickClient({ ...quickClient, apellido1: e.target.value })
                        }
                        className="w-full bg-black/20 border border-slate-700 rounded px-3 py-1.5 text-slate-200 text-sm outline-none focus:border-blue-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={quickClient.apellido2}
                        onChange={(e) =>
                          setQuickClient({ ...quickClient, apellido2: e.target.value })
                        }
                        className="w-full bg-black/20 border border-slate-700 rounded px-3 py-1.5 text-slate-200 text-sm outline-none focus:border-blue-500"
                      />
                    </div>
                  ...
                  <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="px-4 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 text-white rounded font-medium disabled:opacity-50 flex items-center gap-1"
                    >
            ...
            {/* TÉCNICO Y FECHA */}
            <div className="bg-[#0a0a1a] border border-slate-800/60 rounded-xl p-5 md:p-6 shadow-sm">
              <h2 className="text-sm font-bold text-slate-200 mb-4 border-b border-slate-800/50 pb-3">
                Técnico Certificador (Autoguardado)
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={tecnico.nif}
                    onChange={(e) => setTecnico({ ...tecnico, nif: e.target.value })}
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm uppercase"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">
                  ...
                  <input
                    ...
                    className="w-full bg-black/20 border border-slate-800 rounded-lg px-3 py-2 text-slate-200 focus:border-indigo-500 outline-none text-sm [color-scheme:dark]"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#0a0a1a] border-2 border-indigo-500/20 rounded-xl p-5 shadow-lg relative overflow-hidden">
              ...
              <h2 className="text-sm font-bold text-white mb-2 flex justify-between items-center relative z-10">
                ...
                <button
                  ...
                  className="hover:bg-red-500/10 text-slate-400 hover:text-red-400 p-1.5 rounded transition-colors text-xs flex items-center gap-1"
                >
                  <Eraser className="w-3 h-3" /> Borrar
                </button>
              </h2>
              <p className="text-[11px] text-slate-400 mb-3 relative z-10">
                Solicita al cliente que firme dentro del recuadro blanco.
              </p>
                ...
                <div className="bg-slate-100/80 px-3 py-1.5 border-t border-slate-300 rounded-b-xl">
                  <p className="text-[10px] text-slate-500 font-medium font-mono truncate">
                    ...
                    {(selectedClient && selectedClient !== "NEW"
                      ? clientsList.find((c) => c.id === selectedClient)?.first_name
                      : quickClient.nombre) || "---"}
                  </p>
                </div>
              </div>
              ...
              <h2 className="text-sm font-bold text-slate-300 mb-3 flex items-center justify-between">
                ...
                <label className="cursor-pointer bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1 rounded text-xs transition-colors flex items-center gap-1">
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="hidden"
                    onChange={handleUploadTechSignature}
                  />
                </label>
              </h2>
              ...
              {techFirmaUrl ? (
                <div className="relative border border-dashed border-slate-700 rounded-lg p-2 flex justify-center h-40 bg-black/30">
                  <img
                    src={techFirmaUrl}
                    alt="Firma Técnico"
                    className="h-full object-contain filter invert mix-blend-screen opacity-90"
                  />
                  <button
